...
Abstract:
	...
	Cube with color, texture and lights
	Per Pixel Lighting On Cube
	Interlessed Application
...
ID3D11Buffer       *g_pID3D11Buffer_IndexBufferCube_sj = NULL;
ID3D11Buffer       *g_pID3D11Buffer_VertexBuffer_Normals_Cube_sj = NULL;
...
ID3D11ShaderResourceView	*g_pID3D11ShaderResourceView_Texture_Cube = NULL;
ID3D11SamplerState	*g_pID3D11SamplerState_Texture_Cube = NULL;
...
Initialize(void)
{
	...
	const char *vertextShaderSourceCode =
		...
		"Vertex_Output main(float4 pos: POSITION, float4 normals:NORMAL, float3 color: COLOR, float2 texcoord: TEXCOORD)" \
		"{" \
	...
	const char * pixelShaderSourceCode =
		...
		"{" \
			...
			"{" \
				...
				"fcolor = ambient + diffused + specular;" \
			
			"}" \
			"else{" \
				"fcolor = float4(1.0f, 1.0f, 1.0f, 1.0f);" \
			"}"

			"texColor = myTexture2D.Sample(mySamplerState, output.texcoord);" \
			"retColor = fcolor * output.color * texColor;" \
			"return retColor;" \
		"}";
	...
	bufferDescSphere.ByteWidth = sizeof(float) * _ARRAYSIZE(fverticesCube);
	bufferDescSphere.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescSphere.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr_sj = g_pID3D11Device_sj->CreateBuffer(&bufferDescSphere, NULL, &g_pID3D11Buffer_VertexBuffer_Position_Cube_sj );
	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::CreateBuffer() failed for Cube Vertex Buffer\n");
		fclose(g_fLogger_sj);
		return hr_sj;
	}
	else
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::CreateBuffer() Succeded For Cube Vertex Buffer\n");
		fclose(g_fLogger_sj);
	}

	D3D11_MAPPED_SUBRESOURCE mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	g_pID3D11Device_Context_sj->Map(g_pID3D11Buffer_VertexBuffer_Position_Cube_sj ,
		0,
	...
	//bufferNormalsSphere.ByteWidth = sizeof(float) * _ARRAYSIZE(fnormalsCube);
	//bufferNormalsSphere.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	//bufferNormalsSphere.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	//hr_sj = g_pID3D11Device_sj->CreateBuffer(&bufferNormalsSphere, NULL, &g_pID3D11Buffer_VertexBuffer_Normals_Cube_sj);
	//if (FAILED(hr_sj))
	//{
	//	fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
	//	fprintf(g_fLogger_sj, "ID3D11Device::CreateBuffer() failed for Cube Normal Buffer\n");
	//	fclose(g_fLogger_sj);
	//	return hr_sj;
	//}
	//else
	//{
	//	fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
	//	fprintf(g_fLogger_sj, "ID3D11Device::CreateBuffer() Succeded For Cube Normal Buffer\n");
	//	fclose(g_fLogger_sj);
	//}

	//ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	//g_pID3D11Device_Context_sj->Map(g_pID3D11Buffer_VertexBuffer_Normals_Cube_sj, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	//memcpy(mappedSubresource.pData, fnormalsCube, sizeof(fnormalsCube));
	//g_pID3D11Device_Context_sj->Unmap(g_pID3D11Buffer_VertexBuffer_Normals_Cube_sj, NULL);
	...
	hr_sj = LoadD3DTexture(L"marble.bmp", &g_pID3D11ShaderResourceView_Texture_Cube);
	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::Failed For Pyramid Texture\n");
		fclose(g_fLogger_sj);
		return(hr_sj);
	}
	else
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::Succeeded For Pyramid Texture\n");
		fclose(g_fLogger_sj);
	}
	...
	hr_sj = g_pID3D11Device_sj->CreateSamplerState(&samplerDesc, &g_pID3D11SamplerState_Texture_Cube);
	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::CreateSamplerState: Failed For Pyramid Texture\n");
		fclose(g_fLogger_sj);
		return(hr_sj);
	}
	else
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::CreateSamplerState(): Succedded For Pyramid Texture\n");
		fclose(g_fLogger_sj);
	}
	...
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::CreateSamplerState: For Pyramid Texture Failed\n");
		fclose(g_fLogger_sj);
		return(hr_sj);
	}
	else
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::CreateSamplerState():  For Pyramid Texture Succedded\n");
		fclose(g_fLogger_sj);
	}
...
Display(void)
{
	...
	CBUFFER constantBuffer;
	g_pID3D11Device_Context_sj->IASetVertexBuffers(
			0, 1, &g_pID3D11Buffer_VertexBuffer_Position_Cube_sj ,
			&stride, &offset
		);

	offset = sizeof(float) * 3;
	stride = sizeof(float) * 11;
	g_pID3D11Device_Context_sj->IASetVertexBuffers(1, 1, &g_pID3D11Buffer_VertexBuffer_Position_Cube_sj, &stride, &offset);
	//g_pID3D11Device_Context_sj->IASetVertexBuffers(1, 1, &g_pID3D11Buffer_VertexBuffer_Normals_Cube_sj, &stride, &offset);

	offset = sizeof(float) * 6;
	stride = sizeof(float) * 11;
	g_pID3D11Device_Context_sj->IASetVertexBuffers(2, 1, &g_pID3D11Buffer_VertexBuffer_Position_Cube_sj, &stride, &offset);

	offset = sizeof(float) * 9;
	stride = sizeof(float) * 11;
	g_pID3D11Device_Context_sj->IASetVertexBuffers(3, 1, &g_pID3D11Buffer_VertexBuffer_Position_Cube_sj, &stride, &offset);


	//g_pID3D11Device_Context_sj->IASetIndexBuffer(g_pID3D11Buffer_IndexBufferCube_sj, DXGI_FORMAT_R16_UINT, 0);

	g_pID3D11Device_Context_sj->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
	// binding texture and sampler as pixel shader resource
	g_pID3D11Device_Context_sj->PSSetShaderResources(0, 1, &g_pID3D11ShaderResourceView_Texture_Cube);
	g_pID3D11Device_Context_sj->PSSetSamplers(0, 1, &g_pID3D11SamplerState_Texture_Cube);
...
{
	// code
	if (g_pID3D11Buffer_IndexBufferCube_sj)
	{
		g_pID3D11Buffer_IndexBufferCube_sj->Release();
		g_pID3D11Buffer_IndexBufferCube_sj = NULL;
	}

	if (g_pID3D11Buffer_VertexBuffer_Position_Cube_sj )
	{
		g_pID3D11Buffer_VertexBuffer_Position_Cube_sj ->Release();
		g_pID3D11Buffer_VertexBuffer_Position_Cube_sj  = NULL;
	}
...
HRESULT
LoadD3DTexture(const wchar_t *textureFileName, ID3D11ShaderResourceView **ppID3D11ShaderResourceView)
{
	// code
	HRESULT	hr_sj;

	hr_sj = DirectX::CreateWICTextureFromFile(g_pID3D11Device_sj, g_pID3D11Device_Context_sj, textureFileName, NULL, ppID3D11ShaderResourceView);
	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf_s(g_fLogger_sj, "CreateWICTextureFromFile() Failed\n");
		fclose(g_fLogger_sj);
		return(hr_sj);
	}
	else
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf_s(g_fLogger_sj, "CreateWICTextureFromFile() Succeeded\n");
		fclose(g_fLogger_sj);
	}
