...
Abstract:
	Direct3D Application to show rectangle in Perspective View
	This Rectangle drawn using triangle strip
...
HRESULT Initialize(void)
{
	...
	const char *vertextShaderSourceCode =
			...
			"{" \
				"float4 position = mul(WorldViewProjectionMatrix_sj, pos);" \
				"return(position);" \
			"}";
