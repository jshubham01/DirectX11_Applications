...
#define CHECK_IMAGE_WEIGHT 64

BYTE g_checkImage[CHECK_IMAGE_WEIGHT][CHECK_IMAGE_HEIGHT][4];
...
Initialize(void)
{
	...
	//{
	//	fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
	//	fprintf(g_fLogger_sj, "ID3D11Device::Failed For Cube Texture\n");
	//	fclose(g_fLogger_sj);
	//	return(hr_sj);
	//}
	//else
	//{
	//	fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
	//	fprintf(g_fLogger_sj, "ID3D11Device::Succeeded For Cube Texture\n");
	//	fclose(g_fLogger_sj);
	//}
	MakeCheckImage();

	D3D11_TEXTURE2D_DESC desc;
	desc.Width = (UINT)CHECK_IMAGE_WEIGHT;
	desc.Height = (UINT)CHECK_IMAGE_HEIGHT;
	desc.MipLevels = 1;
	desc.ArraySize = 1;
	...
	initData.pSysMem = g_checkImage;
	initData.SysMemPitch = CHECK_IMAGE_WEIGHT * 4;
	initData.SysMemSlicePitch = 0;// CHECK_IMAGE_WEIGHT * CHECK_IMAGE_HEIGHT * 4;

	hr_sj = g_pID3D11Device_sj->CreateTexture2D(&desc, &initData, &tex);
	if (SUCCEEDED(hr_sj) && tex!=0)
	{
		...
		hr_sj = g_pID3D11Device_sj->CreateShaderResourceView(tex, &viewDesc, &g_pID3D11ShaderResourceView_Texture_Cube);
		if ( FAILED(hr_sj) )
		{
			tex->Release();
			return hr_sj;
		...
		//g_pID3D11ShaderResourceView_Texture_Cube = randomTexSRV;
	}
	else
	{
		return hr_sj;
	...
	)
{
	...
	BITMAP	bmp;
	HBITMAP hBitmap;
	bmp = { 0, CHECK_IMAGE_WEIGHT , CHECK_IMAGE_HEIGHT, 4, 1, 1, (PSTR)g_checkImage };
	hBitmap = CreateBitmapIndirect(&bmp);

	IWICImagingFactory *m_pWICFactory = NULL;
	hr_sj = CoCreateInstance(CLSID_WICImagingFactory, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&m_pWICFactory));



	//hr_sj = DirectX::CreateWICTextureFromMemory(
	//	g_pID3D11Device_sj,
	//	g_pID3D11Device_Context_sj,
	//	hBitmap, //(uint8_t *)g_checkImage,
	//	CHECK_IMAGE_WEIGHT * CHECK_IMAGE_HEIGHT * 4,
	//	NULL,
	//	ppID3D11ShaderResourceView
	//);

	//hr_sj = DirectX::CreateWICTextureFromFile(g_pID3D11Device_sj,
	//	g_pID3D11Device_Context_sj,
	//	textureFileName,
	//	NULL, // for Texture Map which blender can gives // sphere map for the
	//	ppID3D11ShaderResourceView);
	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf_s(g_fLogger_sj, "CreateWICTexture() Failed\n");
		fclose(g_fLogger_sj);
		return(hr_sj);
	}
	else
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf_s(g_fLogger_sj, "CreateWICTexture() Succeeded\n");
		fclose(g_fLogger_sj);
	}
...
MakeCheckImage(void)
{
	...
	for (i = 0; i < CHECK_IMAGE_HEIGHT; i++)
	{
		for (j = 0; j < CHECK_IMAGE_WEIGHT; j++)
		{
			...
			//c = 0;
			/*fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
			fprintf_s(g_fLogger_sj, "%d", c);
			fclose(g_fLogger_sj);*/
			c = c * 255;
			g_checkImage[i][j][0] = (BYTE)c;
			g_checkImage[i][j][1] = (BYTE)c;
			g_checkImage[i][j][2] = (BYTE)c;
			g_checkImage[i][j][3] = (BYTE)c;
		}
	}
}
