...
Abstract:
	Direct3D Application to show cube in Perspective View
...
Initialize(void)
{
	...
	const char *vertextShaderSourceCode =
		...
		"{" \
			...
			"{" \
				...
				// "output.diffused_light = mul(ld, mul(kd, max(dot(S, tnorm), 0.0)));" 
			"}" \
			"else{" \
	...
	// normals
	D3D11_BUFFER_DESC bufferNormalsCube;
	ZeroMemory(&bufferNormalsCube, sizeof(D3D11_BUFFER_DESC));
	bufferNormalsCube.Usage = D3D11_USAGE_DYNAMIC;
	bufferNormalsCube.ByteWidth = sizeof(float) * _ARRAYSIZE(fnormalsCube);
	bufferNormalsCube.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferNormalsCube.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr_sj = g_pID3D11Device_sj->CreateBuffer(&bufferNormalsCube, NULL, &g_pID3D11Buffer_VertexBuffer_Normals_Cube_sj);
	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::CreateBuffer() failed for Cube Normals Buffer\n");
		fclose(g_fLogger_sj);
		return hr_sj;
	}
	else
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::CreateBuffer() Succeded For Cube Normals Buffer\n");
		fclose(g_fLogger_sj);
	}
...
void Display(void)
{
	...
	//r1 = XMMatrixRotationX(fAngle);
	r2 = XMMatrixRotationY(fAngle);
	//r3 = XMMatrixRotationZ(fAngle);
	...
	//
	// Filling constant Buffer as Uniforms
	//
	...
	if (true == g_boLight)
	{
		...
		constantBuffer.Ld_sj = XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f); // White
		constantBuffer.Kd_sj = XMVectorSet(0.5f, 0.5f, 0.5f, 1.0f); // Grey
