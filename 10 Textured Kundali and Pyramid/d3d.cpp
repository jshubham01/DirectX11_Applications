...
Abstract:
	Direct3D Application to show cube having Texture in Perspective View
	This cube Rotates around every axis
...
ID3D11Buffer       *g_pID3D11Buffer_VertexBuffer_Textures_Cube_sj = NULL;
ID3D11Buffer       *g_pID3D11Buffer_VertexBuffer_Texture_Pyramid_sj = NULL;
...
ID3D11SamplerState	*g_pID3D11SamplerState_Texture_Pyramid = NULL;
ID3D11ShaderResourceView	*g_pID3D11ShaderResourceView_Texture_Cube = NULL;
ID3D11ShaderResourceView	*g_pID3D11ShaderResourceView_Texture_Pyramid = NULL;
...
Initialize(void)
{
	...
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::CreateBuffer() failed for Pyramid Position\n");
		fclose(g_fLogger_sj);
		return hr_sj;
	}
	else
	...
	// P Y R A M I D S    T E X T U R E S
	...
	bufferDesc_texture.ByteWidth = sizeof(float) * _ARRAYSIZE(ftexcoordsPyramid);
	bufferDesc_texture.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_texture.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr_sj = g_pID3D11Device_sj->CreateBuffer(&bufferDesc_texture, NULL,
		&g_pID3D11Buffer_VertexBuffer_Texture_Pyramid_sj);

	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "D3D11Initialize::CreateBuffer() Failed For Pyramid Texture\n");
		fclose(g_fLogger_sj);
		return hr_sj;
	}
	else
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "D3D11Initialize::CreateBuffer() Succeded For Pyramid Texture\n");
		fclose(g_fLogger_sj);
	}

	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	g_pID3D11Device_Context_sj->Map(g_pID3D11Buffer_VertexBuffer_Texture_Pyramid_sj, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, ftexcoordsPyramid, sizeof(ftexcoordsPyramid));
	g_pID3D11Device_Context_sj->Unmap(g_pID3D11Buffer_VertexBuffer_Texture_Pyramid_sj, NULL);
	...
	bufferDescCube.ByteWidth = sizeof(float) * _ARRAYSIZE(fverticesCube);
	bufferDescCube.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescCube.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr_sj = g_pID3D11Device_sj->CreateBuffer(&bufferDescCube,
		NULL,
	...
	// Cube Textures
	D3D11_BUFFER_DESC bufferDesc_texture_cube;
	ZeroMemory(&bufferDesc_texture_cube, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_texture_cube.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_texture_cube.ByteWidth = sizeof(float) * _ARRAYSIZE(ftextureCube);
	bufferDesc_texture_cube.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_texture_cube.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr_sj = g_pID3D11Device_sj->CreateBuffer(&bufferDesc_texture_cube, NULL,
		&g_pID3D11Buffer_VertexBuffer_Textures_Cube_sj);
	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "D3D11Initialize::CreateBuffer() Failed For Cube Texture\n");
		fclose(g_fLogger_sj);
		return hr_sj;
	}
	else
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "D3D11Initialize::CreateBuffer() Succeded For Cube Texture\n");
		fclose(g_fLogger_sj);
	}

	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	g_pID3D11Device_Context_sj->Map(g_pID3D11Buffer_VertexBuffer_Textures_Cube_sj, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, ftextureCube, sizeof(ftextureCube));
	g_pID3D11Device_Context_sj->Unmap(g_pID3D11Buffer_VertexBuffer_Textures_Cube_sj, NULL);
	...
	hr_sj = LoadD3DTexture(L"Stone.bmp", &g_pID3D11ShaderResourceView_Texture_Pyramid);
	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
	...
	hr_sj = g_pID3D11Device_sj->CreateSamplerState(&samplerDesc, &g_pID3D11SamplerState_Texture_Pyramid);
	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
	...
	hr_sj = LoadD3DTexture(L"Vijay_kundali.bmp", &g_pID3D11ShaderResourceView_Texture_Cube);
	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::Failed For Pyramid Texture\n");
...
void Display(void)
{
	...
	g_pID3D11Device_Context_sj->IASetVertexBuffers(1, 1, &g_pID3D11Buffer_VertexBuffer_Texture_Pyramid_sj, &stride, &offset);

	// binding texture and sampler as pixel shader resource
	g_pID3D11Device_Context_sj->PSSetShaderResources(0, 1, &g_pID3D11ShaderResourceView_Texture_Pyramid);
	g_pID3D11Device_Context_sj->PSSetSamplers(0, 1, &g_pID3D11SamplerState_Texture_Pyramid);
	...
	offset = 0;
	stride = sizeof(float) * 2;
	g_pID3D11Device_Context_sj->IASetVertexBuffers(1, 1, &g_pID3D11Buffer_VertexBuffer_Textures_Cube_sj, &stride, &offset);
...
HRESULT Resize(int iwidth, int iheight)
{
	...
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf_s(g_fLogger_sj, "ID3D11Device::g_pID3D11Device_sj->CreateRenderTargetView() Failed\n");
		fclose(g_fLogger_sj);
		return(hr);
	}
	else
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf_s(g_fLogger_sj, "ID3D11Device::g_pID3D11Device_sj->CreateRenderTargetView() Succeded\n");
		fclose(g_fLogger_sj);
	}
