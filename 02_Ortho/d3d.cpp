...
Abstract:
	Direct3D Application to show triangle in Orthogonal View
...
//
ID3D11PixelShader *g_pID3D11PixelShader = NULL;
ID3D11VertexShader *g_pID3D11VertexShader = NULL;
ID3D11Buffer *g_pID3D11Buffer_VertexBuffer = NULL;
...
XMMATRIX g_OrthographicProjectionMatrix;
...
HRESULT Initialize(void)
{
	...
	hr_sj = g_pID3D11Device->CreateVertexShader(
	...
	hr_sj = g_pID3D11Device->CreatePixelShader(pID3DBlob_PixelShaderCode->GetBufferPointer(),
		pID3DBlob_PixelShaderCode->GetBufferSize(), NULL, &g_pID3D11PixelShader);
	if(FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName,"a+");
	...
	hr_sj = g_pID3D11Device->CreateInputLayout(&inputElementDesc, 
				1,
				pID3DBlob_VertexShaderCode->GetBufferPointer(),
				pID3DBlob_VertexShaderCode->GetBufferSize(),
	...
	hr_sj = g_pID3D11Device->CreateBuffer(&bufferDesc, NULL,
	...
	g_pID3D11DeviceContext->Map(g_pID3D11Buffer_VertexBuffer, NULL,
		D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, vertices, sizeof(vertices));
	g_pID3D11DeviceContext->Unmap(g_pID3D11Buffer_VertexBuffer, NULL);
	...
	hr_sj = g_pID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer, nullptr,
	...
	g_OrthographicProjectionMatrix = XMMatrixIdentity();
...
void Display(void)
{
	...
	UINT offset = 0;
	g_pID3D11DeviceContext->IASetVertexBuffers(0, 1, &g_pID3D11Buffer_VertexBuffer, &stride, &offset);
	...
	XMMATRIX wvpMatrix = worldMatrix * viewMatrix * g_OrthographicProjectionMatrix;
...
HRESULT Resize(int iwidth, int iheight)
{
	...
	if (iwidth <= iheight)
	{
		g_OrthographicProjectionMatrix = XMMatrixOrthographicOffCenterLH(-100.0f,
			100.0f, -100.0f * ((float)iheight / (float)iwidth), 100.0f * ((float)iheight / (float)
				iwidth), -100.0f, 100.0f);
	}
	else
	{
		g_OrthographicProjectionMatrix = XMMatrixOrthographicOffCenterLH(-100.0f *
			((float)iwidth/ (float)iheight), 100.0f * ((float)iwidth / (float)
				iheight), -100.0f, 100.0f, -100.0f, 100.0f);
	}
