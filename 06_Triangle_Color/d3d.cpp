...
ID3D11Buffer       *g_pID3D11Buffer_ColorBuffer_sj = NULL;
...
HRESULT Initialize(void)
{
	...
	hr_sj = g_pID3D11Device_sj->CreateBuffer(&bufferDesc_vertex, NULL,
		&g_pID3D11Buffer_VertexBuffer_sj);

	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::CreateBuffer() For Vertex Failed\n");
		fclose(g_fLogger_sj);
		return hr_sj;
	}
	else
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "ID3D11Device::CreateBuffer() For Positions Succeded For Vertex Buffer()\n");
		fclose(g_fLogger_sj);
	}
	...
	bufferDesc_color.ByteWidth = sizeof(float) * _ARRAYSIZE(color);
	...
	hr_sj = g_pID3D11Device_sj->CreateBuffer(&bufferDesc_color, NULL,
		&g_pID3D11Buffer_ColorBuffer_sj);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	g_pID3D11Device_Context_sj->Map(g_pID3D11Buffer_ColorBuffer_sj, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, color, sizeof(color));
	g_pID3D11Device_Context_sj->Unmap(g_pID3D11Buffer_ColorBuffer_sj, NULL);
...
void Display(void)
{
	...
	g_pID3D11Device_Context_sj->IASetVertexBuffers(1, 1, &g_pID3D11Buffer_ColorBuffer_sj, &stride, &offset);
	...
	rotationMatrix = XMMatrixRotationY(-fAngle); // +ve = right to left
