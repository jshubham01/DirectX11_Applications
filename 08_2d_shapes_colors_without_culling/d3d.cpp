...
ID3D11Buffer       *g_pID3D11Buffer_VertexBuffer_Position_Rectangle_sj = NULL;
ID3D11Buffer       *g_pID3D11Buffer_VertexBuffer_Colors_Rectangle_sj = NULL;
...
HRESULT Initialize(void)
{
	...
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(fverticesTriangle);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr_sj = g_pID3D11Device_sj->CreateBuffer(&bufferDesc, NULL, &g_pID3D11Buffer_VertexBuffer_sj);
	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
	...
	g_pID3D11Device_Context_sj->Map(g_pID3D11Buffer_VertexBuffer_sj, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, fverticesTriangle, sizeof(fverticesTriangle));
	...
	bufferDesc_color.ByteWidth = sizeof(float) * _ARRAYSIZE(fcolorsTriangle);
	...
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
		fprintf(g_fLogger_sj, "D3D11Initialize::CreateBuffer() Failed For Trianlge Color\n");
		fclose(g_fLogger_sj);
		return hr_sj;
	}
	else
	...
	memcpy(mappedSubresource.pData, fcolorsTriangle, sizeof(fcolorsTriangle));
	...
	bufferDescRectangle.ByteWidth = sizeof(float) * _ARRAYSIZE(fverticesRectangle);
	bufferDescRectangle.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescRectangle.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr_sj = g_pID3D11Device_sj->CreateBuffer(&bufferDescRectangle, NULL, &g_pID3D11Buffer_VertexBuffer_Position_Rectangle_sj);
	if (FAILED(hr_sj))
	{
		fopen_s(&g_fLogger_sj, g_szLogFileName, "a+");
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	g_pID3D11Device_Context_sj->Map(g_pID3D11Buffer_VertexBuffer_Position_Rectangle_sj, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, fverticesRectangle, sizeof(fverticesRectangle));
	g_pID3D11Device_Context_sj->Unmap(g_pID3D11Buffer_VertexBuffer_Position_Rectangle_sj, NULL);
	...
	bufferDesc_color_rectangle.ByteWidth = sizeof(float) * _ARRAYSIZE(fcolorsRectangle);
	...
	hr_sj = g_pID3D11Device_sj->CreateBuffer(&bufferDesc_color_rectangle, NULL,
		&g_pID3D11Buffer_VertexBuffer_Colors_Rectangle_sj);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	g_pID3D11Device_Context_sj->Map(g_pID3D11Buffer_VertexBuffer_Colors_Rectangle_sj, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, fcolorsRectangle, sizeof(fcolorsRectangle));
	g_pID3D11Device_Context_sj->Unmap(g_pID3D11Buffer_VertexBuffer_Colors_Rectangle_sj, NULL);
...
void Display(void)
{
	...
	offset = 0;
	stride = sizeof(float) * 3;
	g_pID3D11Device_Context_sj->IASetVertexBuffers(0, 1, &g_pID3D11Buffer_VertexBuffer_Position_Rectangle_sj, &stride, &offset);

	offset = 0;
	stride = sizeof(float) * 3;
	g_pID3D11Device_Context_sj->IASetVertexBuffers(1, 1, &g_pID3D11Buffer_VertexBuffer_Colors_Rectangle_sj, &stride, &offset);
