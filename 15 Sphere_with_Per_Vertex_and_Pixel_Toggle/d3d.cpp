...
Initialize(void)
{
	...
	const char *vertextShaderSourceCode_PV =
		...
		"lightDirection = (float3)normalize(lightPosition - eyeCoordinates);" \
		"tn_dot_ld = max(dot(tnorm, lightDirection), 0.0);" \

		"ambient = la * ka;" \
		"diffused = ld * kd * tn_dot_ld;" \
		"reflectionVector = reflect(-lightDirection, tnorm);" \
		"viewerVector = -normalize(eyeCoordinates.xyz);" \
		"specular = ls * ks * pow(max(dot(reflectionVector, viewerVector), 0.0), materialShininess);" \
